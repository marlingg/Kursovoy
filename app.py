...
#
# Версія оновлена під виправлену модель (без data leakage):
# фінальний набір ознак - 16 предикторів (machine_status, anomaly_flag,
# predicted_remaining_life, downtime_risk, failure_type_enc виключено,
# оскільки виявились похідними від цільової змінної).
...
# Очікувана кількість ознак фінальної моделі (без leakage-полів)
N_FEATURES_EXPECTED = 16
if len(FEATURES) != N_FEATURES_EXPECTED:
    st.warning(
        f'Увага: завантажена модель очікує {len(FEATURES)} ознак, '
        f'а не {N_FEATURES_EXPECTED}. Перевірте відповідність model_artifacts.pkl.'
    )
...
def build_input_row(params: dict) -> pd.DataFrame:
    ...
    Модель навчена лише на первинних сенсорних даних та похідних від них
    ознаках (часові, rolling, лагові, взаємодія) - без службових полів
    machine_status / anomaly_flag / predicted_remaining_life / downtime_risk /
    failure_type_enc, які були виключені через data leakage.
    """
    now = datetime.now()
    row = {
        ...
        'month':                       now.month,
        # rolling / lag - при одиничному вводі прирівнюються до поточного значення
...
tab_pred, tab_analysis, tab_info = st.tabs(
    ['Прогноз', 'Аналіз параметрів', 'Про модель']
)
...
with tab_pred:
    if not predict_btn:
        ...
        col_c.markdown('- RandomForestClassifier\n- 16 ознак (сенсорні + rolling/lag)\n- Accuracy 0.892 / ROC-AUC 0.730')
        ...
        with col3:
            st.metric('Рівень ризику', risk_lbl)
        with col4:
        ...
        with col_t:
            ...
            signals = pd.DataFrame([
                ...
                {'Параметр': 'Вологість',
                 'Значення': f'{humidity:.1f} %',
                 'Норма': '20-90 %',
                 'Статус': signal_status(humidity, high=90, low=20)},
            ])
        ...
        st.caption(
            'Модель пропускає частину реальних випадків потреби в обслуговуванні '
            '(recall для класу "потрібно" ≈ 0.46) - прогноз варто використовувати '
            'як допоміжний сигнал, а не як єдину підставу для рішення.'
        )
...
with tab_analysis:
    ...
    categories = ['Температура', 'Вібрація', 'Вологість', 'Тиск', 'Енергія']
    norm_vals = [
        temperature / 115 * 100,
        vibration   / 100 * 100,
        humidity    / 100 * 100,
    ...
    thresholds = [90/115*100, 70/100*100, 90, 80, 25/30*100]
...
- Real-Time IoT-Driven Production System Dataset (Kaggle)
- Загальний розмір після об'єднання: **102 460 записів**

**Модель:**
- Алгоритм: **RandomForestClassifier** (n_estimators=100, max_depth=8, min_samples_leaf=2)
- Кількість ознак: **16** (сенсорні + часові + rolling + лагові + взаємодія)
- Цільова змінна: `maintenance_required` (0/1)

**Метрики на тестовій вибірці:**
- Accuracy = **0.892**
- ROC-AUC = **0.730**
- Recall (клас "потрібно") = **0.457**
""")
    with col_i2:
        st.markdown("""
**Ключові ознаки (ТОП-3):**
1. `temperature` - температура (~69.0% важливості)
2. `vibration` - вібрація (~20.4%)
3. `temp_x_vib` - взаємодія температури та вібрації (~7.4%)
...
**Примітка:** на початковому етапі моделювання було виявлено data leakage -
п'ять ознак (machine_status, anomaly_flag, predicted_remaining_life,
downtime_risk, failure_type_enc) виявились похідними від цільової змінної
і давали штучно завищені метрики (Accuracy = ROC-AUC = 1.00). Поточна
модель навчена без них, на первинних сенсорних даних.
""")
    ...
    proc_steps = pd.DataFrame([
        ...
        {'Крок': '2. Імпутація пропусків', 'Деталі': 'Медіана для числових ознак'},
        {'Крок': '3. Фільтрація аномалій', 'Деталі': 'IQR-winsorization'},
        ...
        {'Крок': '7. Похідні ознаки',      'Деталі': 'temp x vib, energy / pressure'},
        {'Крок': '8. Масштабування',       'Деталі': 'StandardScaler (для лінійних моделей)'},
        {'Крок': '9. Відбір ознак',        'Деталі': 'Виключено 5 ознак з data leakage'},
    ])
