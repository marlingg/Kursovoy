# ── 3.2 Код Streamlit-застосунку (app.py) ────────────────────────────────────
# streamlit_code = '''
...
if predict_btn:
    ...
    with col1:
        st.metric('🔮 Прогноз', 'ПОТРІБНЕ' if pred else 'НЕ ПОТРІБНЕ',
                  delta='Обслуговування' if pred else 'Продовжити роботу')
    with col2:
        st.metric('📊 Ймовірність обслуговування', f'{prob:.1%}')
    with col3:
    ...
    fig_gauge = go.Figure(go.Indicator(
        mode='gauge+number',
        value=prob * 100,
        title={'text': 'Ймовірність обслуговування (%)', 'font': {'size': 16}},
        gauge={
            'axis': {'range': [0, 100]},
    ...
    st.info('👈 Введіть параметри обладнання в бічній панелі та натисніть «Прогнозувати»')
...
print('app.py збережено. Запуск: streamlit run app.py')